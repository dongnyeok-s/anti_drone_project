...
% 제목 및 저자 정보
% --------------------------------------------------------
\title{\Large\textbf{소부대 대드론 C2 시뮬레이터: \\ 다중 센서 융합 및 다요소 위협 점수 기반 자동 교전 시스템}}
...
\subsection{Time-Critical Nature of Operations}
...
\subsection{Need for Automated Decision Support}
...
\subsection{Detection and Classification Systems}
...
\subsection{Multi-Sensor Fusion Approaches}
...
\subsection{Engagement and Interception}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/paper/system_arch.png}
...
\subsection{Sensor Noise Modeling}
...
\subsection{Multi-factor Threat Score}
...
\end{itemize}
교전 임계값은 Youden 지수 $J = \text{TPR} - \text{FPR}$을 최대화하는 지점에서 결정되었다. 최적 임계값에서 AUC-ROC 0.976, Youden Index 0.86을 달성하였다.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/paper/roc_curve_youden.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/paper/detection_comparison.png}
    \caption{탐지 성능 비교. 탐지율 - Fusion 97.0\% vs Baseline 72.8\% (a), 탐지 지연 - Fusion 4.2 s vs Baseline 7.8 s (b). 오차 막대는 95\% 부트스트랩 신뢰구간.}
    \label{fig:detection}
\end{figure}

검증된 2D 시뮬레이션 환경(20회 반복 $\times$ 3 시나리오)에서 성능을 평가하였다.
...
\begin{tabular}{lccc}
\toprule
\textbf{모드} & \textbf{탐지율 (\%)} & \textbf{지연 (s)} & \textbf{95\% CI} \\
\midrule
\rowcolor{bestrow}
\textbf{Fusion*} & \textbf{96.3} & \textbf{4.2} & [94.1, 98.2] \\
Baseline & 73.2 & 7.8 & [69.8, 76.5] \\
No-EO & 84.7 & 5.9 & [81.2, 88.1] \\
No-Acoustic & 89.1 & 5.2 & [85.9, 92.3] \\
\bottomrule
\multicolumn{4}{l}{\small \textit{Note: Followed by asterisks are significantly different}} \\
\multicolumn{4}{l}{\small \textit{(paired t-test, *: $P < 0.001$)}} \\
\end{tabular}
\label{tab:detection}
\end{table}

제안된 융합 모드는 기준 대비 탐지율을 23\%p 향상시켰으며, 탐지 지연을 4.2 s로 단축시켰다. 이는 초기 대응 시간을 확보하는 데 결정적이다.

\subsection{Classification Performance}
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/paper/pr_curve.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/paper/classification_performance.png}
...
\subsection{Interception Performance}
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/paper/guidance_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/paper/guidance_comparison.png}
    \caption{유도 기법별 요격 성능 비교. 전체 성공률 - PN 84.7\% vs Pure Pursuit 66.9\% (a), 회피 모드 성공률 (b), 평균 미스 거리 (c).}
    \label{fig:interception}
\end{figure}
...
민간 드론 오탐률의 감소(3.2\%)는 교전 규칙(ROE) 적용 시 지휘관의 심리적 부담을 크게 줄여준다. 기존 단일 센서 기반 시스템에서 18.7\%에 달했던 민간 오탐률은 다중 센서 융합을 통해 82\% 감소하였으며, 이는 실전 환경에서 부수적 피해(Collateral Damage) 위험을 현저히 낮출 수 있음을 의미한다.

자동 위협 평가 파이프라인은 수동 분석 대비 처리 단계를 줄여, 운용자가 ``승인/거부''만 판단하면 되는 간소화된 의사결정 흐름을 제공한다. 이러한 설계는 비숙련 운용자도 짧은 교육 기간 내에 시스템을 효과적으로 운용할 수 있게 하며, 소부대 환경에서 흔히 발생하는 인력 순환 문제에 대한 대응책이 될 수 있다.

태블릿 기반의 경량 C2 인터페이스는 소부대 지휘소에서 상황 인식을 지원하도록 설계되었으며, OODA(Observe-Orient-Decide-Act) 루프 가속화에 기여할 것으로 기대된다. 특히 평균 3.1 s의 교전 지연은 15 m/s로 접근하는 드론 기준 약 47 m의 추가 대응 거리를 확보하게 하여, 소부대 생존성 향상에 직접적으로 기여한다.

\subsection{Sensor Fusion Effectiveness}
절제 연구 결과는 다중 센서 융합의 중요성을 명확히 보여준다. EO 센서 제거 시 F1 점수가 11\% 하락하고 민간 오탐률이 360\% 증가한 반면, 음향 센서 제거 시에는 행동 분류 정확도가 35\% 하락하였다. 이는 각 센서가 상호 보완적인 역할을 수행하며, 단일 센서 장애 시에도 시스템이 일정 수준의 성능을 유지할 수 있는 내결함성(Fault Tolerance)을 제공함을 시사한다.

레이더는 장거리 탐지 및 속도 추정에, 음향 센서는 전방위 탐지 및 행동 패턴 분석에, EO 센서는 시각적 분류 및 피아식별에 각각 핵심적인 역할을 담당한다. 본 연구의 가중 융합 방식은 각 센서의 신뢰도를 동적으로 반영하여 환경 조건 변화에 적응적으로 대응할 수 있다.

\subsection{Guidance and Hard-Kill}
비례항법의 높은 선로각 추종 성능($\dot{\lambda}$ 오차 0.15 rad/s)은 하드킬 요격체계의 필수 조건이다. 시뮬레이션 결과는 단순한 추적 알고리즘(Pure Pursuit)으로는 고기동 드론 무력화가 불가능함을 시사하며, 저가형 요격 드론이라도 적절한 유도 법칙이 적용되면 효과적인 방어 수단이 될 수 있음을 보여준다.

특히 회피 기동(EVADE 모드) 표적에 대해 PN 유도가 76.3\%의 성공률을 달성한 반면, Pure Pursuit은 45.2\%에 그쳤다. 이 31\%p의 성능 차이는 고기동 위협 대응에서 유도 알고리즘 선택의 중요성을 강조한다. 평균 미스 거리 2.1 m는 근접신관(Proximity Fuze) 기반 탄두의 유효 반경 내에 충분히 진입 가능한 수준이며, 직접 충돌(RAM) 요격의 가능성도 확보한다.

\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/paper/trajectory.png}
    \caption{회피 표적에 대한 PN(청색)과 Pure Pursuit(적색) 궤적 비교. PN은 곡선 경로로 표적을 효과적으로 추적한다.}
    \label{fig:trajectory}
\end{figure}

\subsection{Comparison with Existing Systems}
기존의 고성능 C-UAS 체계들은 대형 레이더와 전문 운용 인력을 요구하여 소부대 운용에 부적합하다. 본 연구에서 제안한 시스템은 상용 센서와 경량 알고리즘만으로 96.3\%의 탐지율과 84.7\%의 요격 성공률을 달성하여, 비용 대 효과 측면에서 소부대 맞춤형 솔루션의 실현 가능성을 입증하였다. 이는 NATO의 소부대 C-UAS 통합 전략\cite{60}에서 강조하는 ``분산형, 저비용, 신속 배치'' 요구사항과 부합한다.

\subsection{Limitations}
본 연구는 2D 시뮬레이션 환경에서 수행되었으며, 다음과 같은 한계가 존재한다. 첫째, 실제 센서 노이즈 특성과 3D 공간에서의 기동 역학은 추가 검증이 필요하다. 둘째, 다중 요격 드론 간의 협업 및 표적 할당 시나리오는 본 연구의 범위에 포함되지 않았다. 셋째, 전술 통신망의 지연 및 장애 상황이 시스템 성능에 미치는 영향은 반영되지 않았다. 넷째, GPS 재밍이나 스푸핑과 같은 전자전 환경에서의 시스템 강건성은 별도의 연구가 필요하다.
